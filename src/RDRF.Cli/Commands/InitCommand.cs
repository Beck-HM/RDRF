...
public class InitCommand : Command
{
    public InitCommand() : base("init", "Initialize an RDRF storage backend")
    {
        ...
        SetAction((ParseResult parseResult) =>
        {
            ...
            if (count != 1)
            {
                AnsiConsole.MarkupLine("[red]Error: exactly one of -rest, -key, or -path must be specified[/]");
                return 1;
            }
            string? type = null;
            string? rawArgs = null;
            ...
            if (entry == null)
            {
                AnsiConsole.MarkupLine("[red]Error: invalid parameters. Usage:[/]");
                AnsiConsole.MarkupLine("[cyan]  -rest name:<n> & api_url:<url> & token:<t>[/]");
                AnsiConsole.MarkupLine("[cyan]  -key  name:<n> & endpoint:<e> & bucket:<b> & access_key:<k> & secret_key:<s>[/]");
                AnsiConsole.MarkupLine("[cyan]  -path name:<n> & base_path:<p>[/]");
                return 1;
            }

            ConfigManager.AddBackend(entry);
            AnsiConsole.MarkupLine($"[green]Backend '{entry.Name.EscapeMarkup()}' ({type}) added to rdrf_config.yaml[/]");
            return 0;
        });
    }
