...
public class LzmsAlgorithm : ICompressionAlgorithm
{
    private const uint COMPRESS_ALGORITHM_LZMS = 5;

    public string Name => Constants.CompressionLzms;
    ...
    {
        AssumeWindows();
        CreateCompressor(COMPRESS_ALGORITHM_LZMS, IntPtr.Zero, out var handle);
        try
        {
            ...
            if (!Compress(handle, data, us, buf, cbs, out var cs))
                throw new InvalidDataException("LZMS compression failed");
    ...
    public byte[] Decompress(byte[] data)
    {
        ...
        CreateDecompressor(COMPRESS_ALGORITHM_LZMS, IntPtr.Zero, out var handle);
        try
        {
            ...
            if (!Decompress(handle, data[8..(8 + compressedSize)], cs, buf, us, out _))
                throw new InvalidDataException("LZMS decompression failed");
            return buf;
        }
    ...
    private static void AssumeWindows()
    {
        if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            throw new PlatformNotSupportedException("LZMS requires Windows");
    }
}
